create table tb_cliente (id int8 generated by default as identity, email varchar(255), nome varchar(255), primary key (id));
create table tb_compra (quantidade int4, cliente_id int8 not null, passagem_id int8 not null, primary key (cliente_id, passagem_id));
create table tb_passagem (id int8 generated by default as identity, preco float8, destino varchar(255), imagem varchar(255), primary key (id));
alter table if exists tb_compra add constraint FKiaog6v0ghnnqryhvp3bj9b2l1 foreign key (cliente_id) references tb_cliente;
alter table if exists tb_compra add constraint FK3mpxj99s03bvgc7b8wrdhbk1o foreign key (passagem_id) references tb_passagem;
